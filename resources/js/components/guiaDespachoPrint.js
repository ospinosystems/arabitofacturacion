...
(function () {
    ...
    // 1) Medir. Primero se renderiza la tabla completa con layout automático para obtener los
    //    anchos de columna que acomodan TODO el contenido; luego se vuelve a renderizar con esos
    //    anchos fijos (igual que saldrá cada hoja) y se toman las alturas de cada fila.
    ...
    // Reparto igual al de la impresión original: #, Código, Cód. proveedor y Cantidad al ancho de su
    // contenido (los títulos de columna se parten si hace falta) y TODO el resto para Descripción.
    if (FILAS.length) {
        ...
        var resto = ${anchoPx} - (mc[0] + mc[1] + mc[2] + mc[4]);
    ...
    // 2) Repartir filas en páginas. La última página debe dejar lugar para las firmas.
    ...
    // 3) Armar cada hoja con su número propio (base + n° de hoja).
    var html = '';
