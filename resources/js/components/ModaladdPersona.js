...
export default function Modaladdproductocarrito({
  ...
  // Si no hay resultados, rellenar el input de cédula con lo buscado para crear uno nuevo
  useEffect(() => {
    if (searchValue && searchValue.trim() !== "" && personas.length === 0) {
      setclienteInpidentificacion(searchValue.trim());
    }
  }, [searchValue, personas.length]);
            ...
            <div className="bg-white border border-gray-200 rounded-lg">
              ...
              <div ref={tbodypersoInterref} className="divide-y divide-gray-200">
                  {searchValue && searchValue.trim() !== '' && personas.length > 0 ? personas.map((e, i) => (
                    ...
                    // No encontrado: mensaje + formulario con cédula ya rellenada para crear uno nuevo
                    <div className="px-4 py-6">
                      <div className="flex flex-col items-center space-y-3 mb-6 text-center text-gray-500">
                        <svg className="w-12 h-12 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                        </svg>
                        <div className="text-sm">
                          No se encontraron clientes para: "<span className="font-medium">{searchValue}</span>". Crear nuevo con la cédula abajo:
                        </div>
                        <button 
                          onClick={clearSearch}
                        ...
                        <form onSubmit={setPersonaFast} className="space-y-4">
                          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div className="space-y-1">
                              <label htmlFor="identificacion-no-result" className="block text-xs font-medium text-gray-700">
                                C.I./RIF
                              </label>
                              <input
                                type="text"
                            ...
                            <div className="space-y-1">
                              <label htmlFor="nombre-no-result" className="block text-xs font-medium text-gray-700">
                                Nombres y Apellidos
                              </label>
                              <input
                                type="text"
                            ...
                            <div className="space-y-1">
                              <label htmlFor="telefono-no-result" className="block text-xs font-medium text-gray-700">
                                Teléfono
                              </label>
                              <input
                                type="text"
                                className="w-full px-3 py-2 text-sm border border-gray-300 rounded-md focus:ring-1 focus:ring-orange-400 focus:border-orange-400"
                                id="telefono-no-result"
                            ...
                            <div className="space-y-1">
                              <label htmlFor="direccion-no-result" className="block text-xs font-medium text-gray-700">
                                Dirección
                              </label>
                              <input
                                type="text"
                                className="w-full px-3 py-2 text-sm border border-gray-300 rounded-md focus:ring-1 focus:ring-orange-400 focus:border-orange-400"
                                id="direccion-no-result"
                          ...
                          <div className="flex justify-center pt-4">
                            <button 
                              className="flex items-center px-6 py-2 space-x-2 text-sm font-medium text-white transition-colors bg-orange-500 rounded-md hover:text-orange-600 focus:ring-2 focus:ring-orange-400 focus:ring-offset-2" 
                              type="submit"
                            >
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
