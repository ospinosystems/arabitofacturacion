...
class tickera extends Controller
{
    ...
    public function imprimir(Request $req)
    {
        try {
            ...
            $sucursal = sucursal::all()->first();
                ...
                // Marcar pedido como no imprimiendo (solo is_printing, para no sobreescribir ticked que ya actualizó imprimirTicketNormal/Devolucion)
    ...
    function sendFiscalTerminal($parametros,$type,$file,$caja_force=null) {
        ...
        if($codigo_origen=="puntofijo"){
            $nombre_equipo = "caja2";
            $ipReal = gethostbyname($nombre_equipo);
            $response = $this->makeFiscalRequest($ipReal, $parametros, ['origen' => $codigo_origen, 'caja' => $caja, 'nombre_equipo' => $nombre_equipo]);
        }
        if($codigo_origen=="puertopiritu"){
            $nombre_equipo = "caja2";
            $ipReal = gethostbyname($nombre_equipo);
            $response = $this->makeFiscalRequest($ipReal, $parametros, ['origen' => $codigo_origen, 'caja' => $caja, 'nombre_equipo' => $nombre_equipo]);
        }
        if($codigo_origen=="puertolacruz"){
            $nombre_equipo = "caja2";
            $ipReal = gethostbyname($nombre_equipo);
            $response = $this->makeFiscalRequest($ipReal, $parametros, ['origen' => $codigo_origen, 'caja' => $caja, 'nombre_equipo' => $nombre_equipo]);
        }

        if($codigo_origen=="temblador"){
            $nombre_equipo = "caja2";
            $ipReal = gethostbyname($nombre_equipo);
            $response = $this->makeFiscalRequest($ipReal, $parametros, ['origen' => $codigo_origen, 'caja' => $caja, 'nombre_equipo' => $nombre_equipo]);
        }
        ...
        if($codigo_origen=="puertocumarebo"){
            $nombre_equipo = "caja2";
            $ipReal = gethostbyname($nombre_equipo);
            $response = $this->makeFiscalRequest($ipReal, $parametros, ['origen' => $codigo_origen, 'caja' => $caja, 'nombre_equipo' => $nombre_equipo]);
        }
        ...
        if($codigo_origen=="anaco2"){
                $nombre_equipo = "caja2";
                $ipReal = gethostbyname($nombre_equipo);
                $response = $this->makeFiscalRequest($ipReal, $parametros, ['origen' => $codigo_origen, 'caja' => $caja, 'nombre_equipo' => $nombre_equipo]);
        }

        if($codigo_origen=="upata"){
                $nombre_equipo = "caja2";
                $ipReal = gethostbyname($nombre_equipo);
                $response = $this->makeFiscalRequest($ipReal, $parametros, ['origen' => $codigo_origen, 'caja' => $caja, 'nombre_equipo' => $nombre_equipo]);
        }
