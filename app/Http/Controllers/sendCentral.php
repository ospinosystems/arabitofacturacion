...
class sendCentral extends Controller
{
            ...
            if ($response->ok()) {
                ...
                if ($res["estado"]) {
                    ...
                    if (isset($res["tareas"]) && is_array($res["tareas"])) {
                        ...
                        // se denormaliza su descripción y código porque después se elimina.
                        $registrarMovFusion = function($idProducto, $cantidadSumada, $cantidadFinal, $origenProducto) {
                            ...
                            try {
                                ...
                                $origen = 'FUSIÓN: ' . $desc . ' [' . $cod . '] (id ' . $origenProducto->id . ')';
    ...
    private function buildNumeroOrdenComoFrontend(pago_pedidos $pago): string
    {
        ...
        return "{$codigoSucursal}-{$nombreUsuario}-{$pedidoUUID}-{$montoEntero}-{$indiceDebito}";
    }
        ...
        try {
            ...
            if ($response->ok()) {
                ...
                if (!empty($resretur["estado"])) {
                    // CUADRE 2026-06-11 (REVERTIDO) — el check de cuadre estaba MAL ubicado acá.
                    // El export es POST-facturación: el cobro ya se validó en setPagoPedido y en
                    // el interceptor del modelo pedidos (transición 0→1). Además el export REEMPLAZA
                    // los pagos reales por un ficticio tipo=4 0.00001, así que validar cuadre acá
                    // es incoherente. Pedidos viejos legítimamente facturados (o exportados con el
                    // ficticio) quedaban bloqueados sin razón (caso #543238: items $0.7, pagos $0).
                    // La validación de cobro vive donde corresponde: en la facturación, no acá.
                    ...
                    // El interceptor del modelo pedidos NO se dispara acá porque el pedido ya
                    // está en estado=1 (facturado antes de exportar) → no hay transición 0→1.
                    // Si por algún path el pedido estuviera en 0, el ficticio tipo=4 que acabamos
                    // de crear hace que es_transferencia_mercancia=true → el interceptor lo permite.
                    $p->save();
                }
                return $resretur;
            }
